...
import { X, Loader2, CheckCircle, Upload, QrCode, IndianRupee, Copy, Check } from "lucide-react";
...
export default function CheckoutModal({ isOpen, onClose, onSuccess }: CheckoutModalProps) {
    const [step, setStep] = useState<"loading" | "qr" | "upload" | "submitting" | "success">("loading");
                        ...
                        <div className="p-5">
                            {step === "loading" && (
                                <div className="flex flex-col items-center justify-center py-12">
                                    ...
                                    <p className="text-zinc-500 mt-4 font-mono text-sm">Loading checkout...</p>
                                </div>
                            )}
                            ...
                            {step === "success" && (
                                <div className="flex flex-col items-center justify-center py-12">
                                    ...
                                    <h3 className="text-xl font-bold text-green-400 mb-2">Payment Submitted!</h3>
                                    <p className="text-zinc-500 text-sm text-center">
                                        Your payment is being verified. You&apos;ll receive an email once confirmed.
                                    </p>
                                </div>
                            )}
                                        ...
                                        <div className="space-y-2">
                                            {checkoutData.events.map((event, i) => (
                                                <div key={i} className="flex justify-between text-sm">
                                                    ...
                                                    <span className="text-white font-mono">₹{event.fees}</span>
                                                </div>
                                            ))}
                                        </div>
                                        ...
                                        <>
                                            ...
                                            <div className="bg-zinc-900/50 rounded-xl p-3 border border-white/5">
                                                ...
                                                <p className="text-zinc-600 text-xs mt-2">
                                                    Amount: <span className="text-white font-bold">₹{checkoutData.totalAmount}</span>
                                                </p>
                                            </div>
                                            ...
                                            <div>
                                                ...
                                                {screenshotUrl ? (
                                                    ...
                                                    <UploadButton
                                                        ...
                                                        appearance={{
                                                            button: "bg-zinc-800 hover:bg-zinc-700 text-white font-mono text-sm px-4 py-3 rounded-xl border border-zinc-700 w-full",
                            ...
                            {step === "submitting" && (
                                <div className="flex flex-col items-center justify-center py-12">
                                    <Loader2 className="animate-spin text-[#00F0FF]" size={40} />
                                    <p className="text-zinc-500 mt-4 font-mono text-sm">Submitting payment...</p>
                                </div>
                            )}
